...
**3.** A) A table can have multiple Primary Keys. (A table can only have **one** Primary Key, though it can be a composite key spanning multiple columns).
**4.** B) Create Table As Select
**5.** C) The new record is skipped, a warning is generated, and the execution continues without throwing an error.
**6.** B) `ON DUPLICATE KEY UPDATE`
**7.** B) `1`
**8.** D) `DROP TABLE`
**9.** A) `ALTER TABLE users ADD COLUMN birth_date DATE;` (The `COLUMN` keyword is technically optional, but A is the standard correct syntax).
**10.** A) `ALTER TABLE users MODIFY age TINYINT;`
**11.** B) `ENUM`
**12.** C) `4` (The total digits is 6, and 2 are after the decimal point, leaving 4 before it).
**13.** B) `DATE` stores `YYYY-MM-DD`, while `DATETIME` stores `YYYY-MM-DD HH:MM:SS`.
**14.** B) Selecting multiple values from the predefined list at the same time.
**15.** C) `NOW()`
**16.** B) `WHERE`
**17.** C) `WHERE email IS NULL`
**18.** B) Ascending (`ASC`)
**19.** B) Undoes transactions that have not yet been saved to the database.
**20.** B) `AND` is evaluated before `OR`.
