...
```sql
UPDATE staff_update SET salary = 52000 WHERE staff_id = "1";
```

**Q2. Bhavna has been transferred to the Finance department in Indore. Update both her department and city.**
```sql
UPDATE staff_update SET department = "Finance", city = "Indore" WHERE staff_id = 2;
```
...
```sql
UPDATE staff_update SET salary = salary + 4000 WHERE salary < 60000 AND experience >= 3 AND department <> "sales";
```
